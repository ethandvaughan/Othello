...
/* What to do when the srever sends me a log message */
...
socket.on('join_room_response', function(payload) {
	...
	/* If we are being notified that we joined a room, then ignore it */
	if (payload.socket_id == socket.id) {
		return;
	}

	console.log('Join room: '+JSON.stringify(payload));
...
socket.on('player_disconnected', function(payload) {
	...
	/* If we are being notified that we left a room, then ignore it */
	if (payload.socket_id == socket.id) {
		return;
	}
		...
		for(column = 0; column < 8; column++) {
			...
			if(old_board[row][column] != board[row][column]) {
				if(old_board[row][column] == '?' && board[row][column] == ' ') {
					...
					$('#'+row+'_'+column).html('<img src="assets/images/Black-to-White.gif" alt="empty square" />');
				}
				else {
